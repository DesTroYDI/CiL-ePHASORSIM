...
from .io import TileHandler
from .tile_source import OpenStreetMap, TileServer, TileSpec
...
class MapWidget:
    ...
    def __init__(
        ...
        tile_opacity: float = 1.0 # grossh: apply per-tile transparency
    ):
        ...
        self.tile_manager = TileManager(
            ...
            tile_opacity=tile_opacity # grossh: apply per-tile transparency
        )
    ...
    def insert_widget(self) -> int | str:
        ...
        with dpg.drawlist(width=self.width, height=self.height) as self.widget:
            # dpg.draw_rectangle((0, 0), (self.width, self.height)) --> not re-scaled on resize()
            self.tile_manager.add_tile_layer()

        # Bind resize handler to the parent window
        with dpg.item_handler_registry() as self.resize_handler:
            dpg.add_item_resize_handler(callback=self._resize_cb)
        dpg.bind_item_handler_registry(dpg.get_item_parent(self.widget), self.resize_handler)
    ...
    def resize(self, new_width: int, new_height: int):
        """Resize the map widget to new dimensions."""
        # Compute current center before changing anything
        center = self.origin.with_screen_offset(
            self.width / 2,
            self.height / 2,
        ...
        # Recompute origin from center with new viewport size
        self.origin = center.with_screen_offset(
            -new_width / 2,
            -new_height / 2,
            zoom=self.zoom_level,
            resolution=self.tile_manager.tile_server.tile_size,
        ...
        # Resize the drawlist
        dpg.set_item_width(self.widget, new_width)
        dpg.set_item_height(self.widget, new_height)

        # Update tile manager
        self.tile_manager.viewport_size = (new_width, new_height)
        self.tile_manager.set_origin(self.origin, self.zoom_level)
    ...
    def _mouse_wheel_cb(self, sender: int | str, app_data: int) -> None:
        ...
        if not dpg.is_item_hovered(self.widget):
            return
        del sender  # unused
        delta_zoom = app_data
    ...
    def _resize_cb(self, sender, app_data):
        """Callback on widget resize"""
        parent = dpg.get_item_parent(self.widget)
        new_w = dpg.get_item_width(parent)
        new_h = dpg.get_item_height(parent)
...
class TileManager:
    ...
    def __init__(
        ...
        self.tile_opacity = tile_opacity    # grossh: apply per-tile transparency

        self.tile_draw_node_id: int | str = None
        self.overlay_draw_node_id: int | str = None     # UI layer: pandapower map overlay
        self.ui_draw_node_id: int | str = None          # UI layer: legend
    ...
    def add_tile_layer(self):
        ...
        # UI-Layer: PandaPower-Netzmodell
        overlay_layer = dpg.add_draw_layer(label="overlay")
        self.overlay_draw_node_id = dpg.add_draw_node(parent=overlay_layer)

        # UI-Layer: Legende
        legende_layer = dpg.add_draw_layer(label="legende")
        self.ui_draw_node_id = dpg.add_draw_node(parent=legende_layer)

    def draw_layer(self):
        """Update tile layer, if there are too few tiles displayed"""
        translation = [sum(i) for i in zip(self.origin_offset, self.last_drag)]

        dpg.apply_transform(
            self.tile_draw_node_id,
            dpg.create_translation_matrix(translation)
        )
        # Draw UI layer for pandapower network model
        if self.overlay_draw_node_id is not None:
            dpg.apply_transform(self.overlay_draw_node_id, 
                                dpg.create_translation_matrix(translation) )
    ...
    def draw_tile(self, tile_spec: TileSpec):
        ...
        if tile.draw_image(parent=self.tile_draw_node_id, opacity=self.tile_opacity): # grossh: apply per-tile transparency
...
class MapTile:
    ...
    def draw_image(self, parent: int | str, opacity: float = 1.0) -> bool: # grossh: apply per-tile transparency
        """Draw tile
        ...
        self.image_tag = dpg.draw_image(
            ...
            pmin,
            pmax,
            color=(255, 255, 255, int(opacity * 255)),       # grossh: apply per-tile transparency
            parent=parent,
        )
        return True
...
def add_map_widget(
    ...
    Returns:
        tuple[MapWidget, int | str]: MapWidget instance and tag of Dear PyGui drawlist
    """
    ...
    return map_widget, map_widget.insert_widget()